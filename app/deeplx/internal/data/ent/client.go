...
func (c *AccessLogClient) UpdateOneID(id int64) *AccessLogUpdateOne {
...
func (c *AccessLogClient) DeleteOneID(id int64) *AccessLogDeleteOne {
...
func (c *AccessLogClient) Get(ctx context.Context, id int64) (*AccessLog, error) {
	return c.Query().Where(accesslog.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AccessLogClient) GetX(ctx context.Context, id int64) *AccessLog {
...
// QueryOwnerUser queries the owner_user edge of a AccessLog.
func (c *AccessLogClient) QueryOwnerUser(al *AccessLog) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := al.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(accesslog.Table, accesslog.FieldID, id),
...
// QueryAccessLogs queries the access_logs edge of a User.
func (c *UserClient) QueryAccessLogs(u *User) *AccessLogQuery {
	query := (&AccessLogClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		AccessLog, User []ent.Hook
	}
	inters struct {
		AccessLog, User []ent.Interceptor
	}
)
